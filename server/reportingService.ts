...
export class ReportingService {
  ...
  static async createReportConfig(
    userId: string,
    config: Omit<ReportConfig, "id" | "userId">,
  ...
  static async generateWeeklyPerformanceReport(
    userId: string,
  ): Promise<ReportData> {
  ...
  static async generateMonthlyPerformanceReport(
    userId: string,
  ): Promise<ReportData> {
    const endDate = new Date();
    const startDate = new Date();
  ...
  static async generateEmailDigest(userId: string): Promise<string> {
    ...
    const upcomingEvents = await CalendarIntegrationService.getUpcomingEvents(
      userId,
      3,
    );
  ...
  private static async generatePDFReport(
    ...
    doc
      .fontSize(12)
      .text(
        `Period: ${data.metadata.period.start.toDateString()} - ${data.metadata.period.end.toDateString()}`,
        50,
        100,
      );
    ...
    doc
      .fontSize(12)
      .text(
        `Total Value: $${data.portfolioSummary.totalValue.toLocaleString()}`,
        50,
        160,
      );
    doc.text(
    ...
    doc.text(
      `Best Performer: ${data.portfolioSummary.bestPerformer.symbol} (+${data.portfolioSummary.bestPerformer.return}%)`,
      50,
      200,
    );
    doc.text(
      `Worst Performer: ${data.portfolioSummary.worstPerformer.symbol} (${data.portfolioSummary.worstPerformer.return}%)`,
      50,
      220,
    );
    ...
    doc
      .fontSize(12)
      .text(`Sharpe Ratio: ${data.performanceMetrics.sharpeRatio}`, 50, 280);
    ...
    doc
      .fontSize(12)
      .text(`Total Trades: ${data.tradeAnalysis.totalTrades}`, 50, 400);
    doc.text(
    ...
    doc.text(
      `Total Profit: $${data.tradeAnalysis.totalProfit.toLocaleString()}`,
      50,
      440,
    );
    doc.text(
      `Total Loss: $${data.tradeAnalysis.totalLoss.toLocaleString()}`,
      50,
      460,
    );
  ...
  private static async sendReportEmail(
    ...
    const mailOptions = {
      ...
      html:
        config.format === "HTML"
          ? reportResult.html
          : await this.generateEmailDigest(config.userId),
      attachments:
        config.format === "PDF"
          ? [
              {
