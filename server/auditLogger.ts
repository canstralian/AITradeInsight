...
export interface SecurityEvent {
  type: 'LOGIN_ATTEMPT' | 'LOGIN_SUCCESS' | 'LOGIN_FAILURE' | 'MFA_SETUP' | 'MFA_VERIFY' | 
        'TRADE_ATTEMPT' | 'TRADE_SUCCESS' | 'TRADE_FAILURE' | 'RATE_LIMIT_HIT' | 
        'SUSPICIOUS_ACTIVITY' | 'PASSWORD_CHANGE' | 'ACCOUNT_LOCKED';
  severity: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL';
  userId?: string;
  details: Record<string, any>;
}
  ...
  static async logAction(
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(`[AUDIT] ${severity}: ${action} on ${resource} by ${userId}`, details);
      }
    } catch (error) {
