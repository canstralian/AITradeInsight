...
export const tradingApi = {
  ...
  searchStocks: (query: string) =>
    apiRequest<any[]>(`/stocks/search?q=${encodeURIComponent(query)}`),
  ...
  removeFromWatchlist: (stockId: number) =>
    apiRequest<void>(`/watchlist/${stockId}`, {
      method: "DELETE",
    }),

  // AI Predictions endpoints
  getAiPredictions: () => apiRequest<any[]>("/predictions"),
  getAiPrediction: (stockId: number) =>
    apiRequest<any>(`/predictions/${stockId}`),

  // Trading Signals endpoints
  getTradingSignals: () => apiRequest<any[]>("/signals"),
  getTradingSignalsByStock: (stockId: number) =>
    apiRequest<any[]>(`/signals/${stockId}`),
  ...
  getCryptoRadar: (type?: string) =>
    apiRequest<any[]>(`/crypto/radar${type ? `?type=${type}` : ""}`),
  getCryptoValidation: (coinId: number) =>
    apiRequest<any>(`/crypto/validation/${coinId}`),

  // Automated trading endpoints
  getAutomatedTrades: () => apiRequest<any[]>("/trading/automated"),
  createAutomatedTrade: (trade: any) =>
    apiRequest<any>("/trading/automated", {
      method: "POST",
      body: JSON.stringify(trade),
    }),
  updateAutomatedTrade: (tradeId: number, updates: any) =>
    apiRequest<any>(`/trading/automated/${tradeId}`, {
      method: "PUT",
      body: JSON.stringify(updates),
    }),
  cancelAutomatedTrade: (tradeId: number) =>
    apiRequest<void>(`/trading/automated/${tradeId}`, {
      method: "DELETE",
    }),

  // Trading strategy search endpoints
  searchTradingStrategies: (query: string) =>
    apiRequest<any[]>(`/strategies/search?query=${encodeURIComponent(query)}`),
  ...
  connectBroker: (credentials: any) =>
    apiRequest<any>("/brokers/connect", {
      method: "POST",
      body: JSON.stringify(credentials),
    }),
  getBrokerAccounts: () => apiRequest<any[]>("/brokers/accounts"),
  syncBrokerAccount: (accountId: string) =>
    apiRequest<any>(`/brokers/sync/${accountId}`, { method: "POST" }),
  getConsolidatedPortfolio: () => apiRequest<any>("/brokers/portfolio"),
  executeTrade: (tradeData: any) =>
    apiRequest<any>("/brokers/trade", {
      method: "POST",
  ...
  setupCalendarAlerts: (preferences: any) =>
    apiRequest<any>("/calendar/alerts", {
      method: "POST",
      body: JSON.stringify(preferences),
    }),

  // Reporting endpoints
  createReport: (config: any) =>
    apiRequest<any>("/reports/create", {
      method: "POST",
      body: JSON.stringify(config),
    }),
  generateReport: (reportId: string) =>
    apiRequest<any>(`/reports/generate/${reportId}`),
