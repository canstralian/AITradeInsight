...
export function GetStarted() {
  ...
  const validateStep2 = (): boolean => {
    ...
    if (formData.investmentGoals.length === 0) newErrors.investmentGoals = ['At least one investment goal is required'] as any;
    if (!formData.initialInvestment) newErrors.initialInvestment = 'Initial investment amount is required';
    if (!formData.tradingStyle) newErrors.tradingStyle = 'Trading style is required';
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center p-4">
      <div className="w-full max-w-2xl">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">Get Started with AI Trading</h1>
          <p className="text-gray-400">Complete your profile to access personalized trading insights</p>
        </div>

        {/* Progress Bar */}
        <div className="mb-8">
          <div className="flex justify-between text-sm text-gray-400 mb-2">
            <span className={currentStep >= 1 ? 'text-purple-400' : ''}>Personal Information</span>
            <span className={currentStep >= 2 ? 'text-purple-400' : ''}>Trading Profile</span>
          </div>
        ...
        <Card className="bg-gray-800 border-gray-700">
          <CardHeader>
            <CardTitle className="text-white">
              Step {currentStep} of 2: {currentStep === 1 ? 'Personal Information' : 'Trading Profile'}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            {currentStep === 2 && (
              <>
                ...
                <div>
                  <Label className="text-gray-300">Investment Goals * (Select all that apply)</Label>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
                    {[
                      ...
                      <div key={goal} className="flex items-center space-x-2">
                        ...
                        />
                        <Label htmlFor={goal} className="text-gray-300 text-sm">{goal}</Label>
                      </div>
                    ))}
                  </div>
                  {errors.investmentGoals && (
                    <p className="text-red-400 text-sm mt-1">{errors.investmentGoals[0]}</p>
                  )}
                </div>

                <div>
                  <Label className="text-gray-300">Initial Investment Amount *</Label>
                  <Select value={formData.initialInvestment} onValueChange={(value) => updateFormData('initialInvestment', value)}>
                    <SelectTrigger className="bg-gray-700 border-gray-600 text-white">
                ...
                <div>
                  ...
                  {errors.tradingStyle && (
                    <p className="text-red-400 text-sm mt-1">{errors.tradingStyle}</p>
                  )}
                </div>
                ...
                <div className="space-y-3">
                  ...
                  {errors.termsAccepted && (
                    <p className="text-red-400 text-sm">You must accept the terms and conditions</p>
                  )}
                  ...
                  <div className="flex items-start space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => updateFormData('newsletterSubscription', checked)}
                    />
            ...
            <div className="flex justify-between pt-6">
              <Button
                variant="outline"
                onClick={currentStep === 1 ? () => setLocation('/') : handlePrevious}
        ...
        <div className="text-center mt-6">
          <p className="text-gray-500 text-sm">
            ...
            <a href="mailto:support@aitrading.com" className="text-purple-400 hover:underline">
