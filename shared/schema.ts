...
export const aiPredictionsTable = pgTable("ai_predictions", {
  id: serial("id").primaryKey(),
  stockId: integer("stock_id").notNull(),
  prediction24h: numeric("prediction_24h", {
    precision: 10,
    scale: 2,
  }).notNull(),
...
export const marketSentimentTable = pgTable("market_sentiment", {
  ...
  bullishPercent: numeric("bullish_percent", {
    precision: 5,
    scale: 2,
  }).notNull(),
  bearishPercent: numeric("bearish_percent", {
    precision: 5,
    scale: 2,
  }).notNull(),
  neutralPercent: numeric("neutral_percent", {
    precision: 5,
    scale: 2,
  }).notNull(),
  lastUpdated: timestamp("last_updated").defaultNow().notNull(),
...
export const insertTradingSignalSchema =
  createInsertSchema(tradingSignalsTable);
export const insertMarketSentimentSchema =
  createInsertSchema(marketSentimentTable);
...
export const selectTradingSignalSchema =
  createSelectSchema(tradingSignalsTable);
export const selectMarketSentimentSchema =
  createSelectSchema(marketSentimentTable);
...
export const tradingLimitsTable = pgTable("trading_limits", {
  ...
  maxPortfolioValue: numeric("max_portfolio_value", {
    precision: 15,
    scale: 2,
  }),
...
export const brokerAccountsTable = pgTable("broker_accounts", {
  ...
  buyingPower: numeric("buying_power", { precision: 15, scale: 2 }).default(
    "0",
  ),
...
export const PredictionSchema = z.object({
  ...
  prediction24h: z
    .string()
    .regex(/^\d+(\.\d{1,2})?$/, "Invalid prediction format"),
  prediction7d: z
    .string()
    .regex(/^\d+(\.\d{1,2})?$/, "Invalid prediction format"),
  confidence: z.string(),
...
export const validateStock = (data: unknown): z.infer<typeof StockSchema> =>
...
export const validatePrediction = (data: unknown): Prediction =>
  PredictionSchema.parse(data);
export const validateTradingSignal = (
  data: unknown,
): z.infer<typeof TradingSignalSchema> => TradingSignalSchema.parse(data);
export const validatePortfolio = (
  data: unknown,
): z.infer<typeof PortfolioSchema> => PortfolioSchema.parse(data);
export const validateWatchlistItem = (
  data: unknown,
): z.infer<typeof WatchlistItemSchema> => WatchlistItemSchema.parse(data);
export const validateMarketSentiment = (
  data: unknown,
): z.infer<typeof MarketSentimentSchema> => MarketSentimentSchema.parse(data);
export const validateRecommendation = (data: unknown): Recommendation =>
